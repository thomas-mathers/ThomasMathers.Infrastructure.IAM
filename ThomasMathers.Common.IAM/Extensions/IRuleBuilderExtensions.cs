...
namespace ThomasMathers.Common.IAM.Extensions
{
    ...
    {
        public static IRuleBuilder<T, string> Username<T>(this IRuleBuilder<T, string> ruleBuilder, UserSettings userSettings)
        {
            return ruleBuilder
                .NotEmpty()
        ...
        public static IRuleBuilder<T, string> Password<T>(this IRuleBuilder<T, string> ruleBuilder, PasswordSettings passwordSettings)
        {
            return ruleBuilder
        ...
        public static IRuleBuilder<T, string> UsesAlphabet<T>(this IRuleBuilder<T, string> ruleBuilder, HashSet<char> alphabet)
        {
            return ruleBuilder.Must(s => s.All(alphabet.Contains)).WithMessage($"Must only use characters from {string.Join(",", alphabet)}");
        }

        public static IRuleBuilder<T, string> UsesAlphabet<T>(this IRuleBuilder<T, string> ruleBuilder, string alphabet)
            => ruleBuilder.UsesAlphabet(new HashSet<char>(alphabet));


        public static IRuleBuilder<T, string> MinimumUniqueCharacters<T>(this IRuleBuilder<T, string> ruleBuilder, int minUniqueCharacters)
        {
            return ruleBuilder.Must(s => s.Distinct().Count() >= minUniqueCharacters).WithMessage($"Must have at least {minUniqueCharacters} unique characters");
        }
        ...
        public static IRuleBuilder<T, string> ContainsNonAlphanumeric<T>(this IRuleBuilder<T, string> ruleBuilder, bool containsNonAlphanumeric)
        {
            ...
            return ruleBuilder.Must(s => s.Any(c => !char.IsLetterOrDigit(c))).WithMessage($"Must have at least one non alphanumeric letter");
        }
    }
}
